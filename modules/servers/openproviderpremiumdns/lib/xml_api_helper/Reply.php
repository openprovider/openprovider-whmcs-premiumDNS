...
class Reply
{
    ...
    protected function _parseReply($str = '')
    {
        ...
        if (!$result)
        {
            \logModuleCall(MODULE_NAME, 'connecting_with_openprovider_error', null, $str, $result, null);
            throw new \Exception('Cannot parse XML');
        }
        $arr = \OpenproviderPremiumDns\lib\xmlapihelper\APITools::convertXmlToPhpArray($str);
    ...
    public function getRaw()
    {
        ...
        $descNode->appendChild(
                $dom->createTextNode(mb_convert_encoding($this->faultString, \OpenproviderPremiumDns\lib\xmlapihelper\APIConfig::$encoding))
        );
        $dataNode = $replyNode->appendChild($dom->createElement('data'));
        \OpenproviderPremiumDns\lib\xmlapihelper\APITools::convertPhpObjToDom($this->value, $dataNode, $dom);
        if (0 < count($this->warnings))
        {
            $warningsNode = $replyNode->appendChild($dom->createElement('warnings'));
            \OpenproviderPremiumDns\lib\xmlapihelper\APITools::convertPhpObjToDom($this->warnings, $warningsNode, $dom);
        }
        return $dom->saveXML();
    }
